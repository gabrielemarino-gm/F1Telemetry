...
{
    public static void aggiungiRecordUtenteDB(String utente, DriversAndCircuit d) 
    {
        try( Connection co = DriverManager.getConnection("jdbc:mysql://localhost:3306/f1telemetry","root","");  
    ...
    public static void eliminaRecordUtenteDB(String utente, DriversAndCircuit d) 
    {
        try(Connection co = DriverManager.getConnection("jdbc:mysql://localhost:3306/f1telemetry","root","");  
    ...
    public static void caricaTabellaDriversDB(String utente, AreaTabellaDrivers tb)
    {        
        tb.eliminaTutteLeRighe();
             ...
             PreparedStatement ps = co.prepareStatement("SELECT * FROM drivers_for_users WHERE user = ?");)
        {
            ...
            while(rs.next())
               tb.inserisciRigaDB(rs.getString("driver"), rs.getString("circuit"), rs.getString("telemetry_type"), rs.getString("file_path"));           
        } 
